...
ws.addEventListener("message", async (ev) => {
  ...
  if (m.type === "auth_ok") {
    main(ws).catch((e) => { console.error(e); process.exit(1); });
    return;
  }
  if (m.type === "auth_invalid") {
    console.error("auth_invalid"); process.exit(1);
  }
  if (m.type === "result" && pending.has(m.id)) {
    const { resolve, reject } = pending.get(m.id);
    pending.delete(m.id);
    if (m.success) resolve(m.result); else reject(new Error(JSON.stringify(m.error)));
  }
});
