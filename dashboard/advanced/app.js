    ...
    function renderStrip(get) {
      ...
      const tVals = [
        ...
        'sensor.jk_pb_bms_temperature_sensor_4',
        'sensor.jk_pb_bms_temperature_sensor_5',
      ].map(id => num(get(id))).filter(n => !isNaN(n));
      ...
      const voltCls = isNaN(volt) ? '' : (volt < 48 ? 'neg' : volt > 58 ? 'neg' : volt > 56 ? 'warn' : '');
    ...
    function renderAdvanced(states, latencyMs) {
      ...
      $('live').innerHTML =
        ...
        row('Errors',                    `${errText || '(none)'} <span class="${errCls}">[bitmask ${numOr(errB,0)}]</span>`, errB, errCls);
      ...
      const SCALE_MV = 0.030;       // visualisation half-window
      const ALERT_MV = 0.030;       // "moderate" imbalance highlight threshold
      const OUTLIER_MV = 0.080;     // "bad" imbalance threshold
      ...
      for (const c of cellSamples) {
        ...
        cellsHtml += `<div class="cell-line ${cls}">`
      ...
      $('cfg').textContent = JSON.stringify({ HA_URL: HA_URL || '(same-origin)', ENTITIES, POLL_MS_ADVANCED, STALE_MS, FEED_MAX, isDemo, tokenLen: TOKEN.length, tokenSet: !TOKEN.startsWith('PASTE_LONG_') }, null, 2);
    }
    ...
    function renderHistory() {
      ...
      drawChart($('chart-soc'),     socB,  { ...baseOpts, lineColor: '#4ade80', fixedMin: 0, fixedMax: 100 });
      drawChart($('chart-current'), curB,  { ...baseOpts, band: true, bandColor: 'rgba(251, 191, 36, 0.18)', lineColor: '#fbbf24', zeroLine: true });
      drawChart($('chart-power'),   pwrB,  { ...baseOpts, band: true, bandColor: 'rgba(251, 191, 36, 0.18)', lineColor: '#fbbf24', zeroLine: true });
      drawChart($('chart-temp'),    tempB, { ...baseOpts, band: true, bandColor: 'rgba(96, 165, 250, 0.18)', lineColor: '#60a5fa' });
    ...
    const chartObserver = new ResizeObserver(() => { if (historyCache) renderHistory(); });
    ['#chart-soc', '#chart-current', '#chart-power', '#chart-temp']
      .forEach(sel => { const el = document.querySelector(sel); if (el) chartObserver.observe(el); });
